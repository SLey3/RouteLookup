...
fr_api = FlightRadar24API(
    config.FLIGHTRADAR_USERNAME, config.FLIGHTRADAR_PASSWORD
)  # noqa: E501
...
class Airline:
    def __init__(self, airline: Dict):
        ...
        self.actype = self.callsign = self.origin = self.destination = None
...
def get_airline(name: str):
    ...
    for airline_dict in fr_api.get_airlines():
        if name == airline_dict["Name"].capitalize():
            airline = airline_dict
            break
        elif name.upper() == airline_dict["Code"]:
            airline = airline_dict
            break
        elif name.upper() == airline_dict["ICAO"]:
            airline = airline_dict
            break
    return airline
...
def _get_origin_airport(flight):
    try:
        ...
        sleep(
            10
        )  # this prevents the CloudflareError through delaying time between # noqa: E501
...
def refresh():
    """
    In the event that a new airline is set (e.g. AAL --> DAL),
    this will delete all text and regenerate the routes
    """
...
def clean_flights(flights: list):
    for flight in flights[:]:
        if (
            flight.destination_airport_iata == "N/A"
            or flight.origin_airport_iata == "N/A"
        ):
    ...
    if isinstance(airline, dict):  # basically if airline is not None
        ...
        with open(FILE_DIR, "w") as file:
            ...
            for flight in flights:
                ...
                file.write(
                    f"A/C TYPE: {aircraft_type}|CALLSIGN: {callsign}|ORIGIN: {origin_icao}|DESTINATION: {destination_icao}\n"  # noqa: E501
                )
