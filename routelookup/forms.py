    ...
    def __call__(self, **kwargs):
        ...
        if config.NAVIGRAPH_ENABLED == "Enabled":
            if self.part == "Enable":
                ...
                params = html_params(
                    disabled=True, aria_current="rg-enable", **kwargs
                )  # noqa: E501
            else:
        ...
        return Markup(
            "<button %s>%s</button>" % (params, escape(self.render_kw["value"]))
        )
    ...
    def add_validator_to(self, attr, *args):
        ...
        if hasattr(validator_attr, "validators"):
            ...
            raise AttributeError(
                "Check again if you inputed the correct attribute name."
            )

    submit_btn = SubmitField(
        id="airline-form-sbmt",
        render_kw={"class": "btn btn-dark mb-3", "value": "Submit"},
    )
...
class Config_FlightRadarUSRForm(FlaskForm):
    email = StringField(
        ...
        validators=[
            Email(
                "check if email is written correctly (correct email format)",
                check_deliverability=True,
            )
        ],
        render_kw={"class": "form-control-sm", "placeholder": "Email"},
    )

    email_submit = SubmitField(
        id="fr-usr-sbmt", render_kw={"class": "btn btn-primary mb-3", "value": "Submit"}
    )
    ...
    def validate_email(form, field):
        try:
            ...
            raise ValidationError(
                "Check if your email is the correct email from FlightRadar24."
            )
...
class Config_FlightRadarPWDForm(FlaskForm):
    pwd = PasswordField(
        id="fr-pwd-input",
        render_kw={"class": "form-control-sm", "placeholder": "Password"},
    )

    pwd_submit = SubmitField(
        id="fr-pwd-sbmt", render_kw={"class": "btn btn-primary mb-3", "value": "Submit"}
    )
    ...
    def validate_pwd(form, field):
        ...
        try:
            ...
            raise ValidationError(
                "Check if your password is the correct password from FlightRadar24"
            )


class Config_NavigraphEnableForm(FlaskForm):
    enable = NavigraphField(
        "Enable",
        id="rg-enable",
        render_kw={"name": "route-generation-true", "value": "Enable", "class": "btn"},
    )

    disable = NavigraphField(
        "Disable",
        id="rg-disable",
        render_kw={
...
class Config_RefreshTable(FlaskForm):
    refresh = SubmitField(
        render_kw={"value": "Refresh", "class": "btn btn-primary mb-3"}
    )
